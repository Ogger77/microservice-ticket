import { OrderStatus } from "@oggerticket/common";
...
it("return a 401 wjen purchasing an order that does not belong to the user", async () => {
...
it("return a 400 when purchasing a cancelled order", async () => {
  ...
  await request(app)
    ...
    .send({ orderId: order.id, token: "asfasf" })
    .expect(400);
});
